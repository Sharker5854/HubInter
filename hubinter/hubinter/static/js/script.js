...
function show_videos_by_tag(tag_name, current_theme) {
    var all_buttons = document.getElementsByClassName('tag-button')
    var tag_button = document.getElementById('tag-btn-' + tag_name)

    if (!tag_button.classList.contains('selected-tag')) { // enable tag
        for (let btn of all_buttons) { // firstly disable all other tag buttons
            btn.classList.remove('selected-tag')
        }
        tag_button.classList.add('selected-tag')
    }


    else { // disable tag
        tag_button.classList.remove('selected-tag')
    }
...
/// ONE MORE AJAX FUNCTION OF add_video.html TEMPLATE ///
